---
name: design-taste
description: Regole anti-slop per il frontend (tipografia, colore, layout, materialita, stati interattivi, checklist pre-flight). Usare prima di scrivere o modificare qualsiasi CSS o markup di interfaccia.
source: https://github.com/leonxlnx/taste-skill (skills/taste-skill, v2)
---

# tasteskill: Anti-Slop Frontend Skill – Core Rules Digest

This document contains the complete frontend design system for building landing pages, portfolios, and redesigns without AI-generated visual clichés.

---

## 0. BRIEF INFERENCE (Read First)

**Before touching code**, infer what the user actually wants by reading:
- Page kind (landing / portfolio / redesign / editorial)
- Vibe words used ("minimalist," "playful," "serious B2B," etc.)
- Reference signals (URLs, screenshots, competitor brands)
- Audience type (B2B, design-conscious consumer, recruiter)
- Existing brand assets (logo, color, type, photography)
- Quiet constraints (accessibility-first, regulated industries, trust-critical)
...
**If the brief is ambiguous, ask ONE clarifying question only.** Never multi-question dumps.

**Anti-Default Discipline:** Do not default to AI-purple gradients, centered hero over dark mesh, three equal feature cards, generic glassmorphism, endless micro-animations, or Inter + slate-900.
...
| **`MOTION_INTENSITY`** | 1–10 | 1 = Static; 10 = Cinematic / Physics-Driven |
| **`VISUAL_DENSITY`** | 1–10 | 1 = Art Gallery / Airy; 10 = Cockpit / Packed Data |

**Baseline:** `8 / 6 / 4` unless the design read overrides.
...
- Redesign (preserve): match existing + 1 motion
- Redesign (overhaul): +2 variance, +2 motion, match density

---

## 2. DESIGN SYSTEM MAP (Don't Invent CSS)

Pick the right foundation. Do not invent CSS for things that have official packages.

| Brief Reads As | Reach For | Why |
|---|---|---|
| Microsoft / enterprise SaaS | `@fluentui/react-components` | Official Fluent UI, tokens, accessibility done |
| Google-ish / Material-flavored | `@material/web` + Material 3 tokens | Official, theme-able |
| IBM-style B2B analytics | `@carbon/react` | Official Carbon, data-density patterns |
| Shopify app surfaces | `polaris.js` web components | Required for Shopify admin UI |
| Atlassian / Jira-style | `@atlaskit/*` + tokens | Official Atlassian DS |
| GitHub devtool / community | `@primer/css` or `@primer/react-brand` | Official Primer |
| UK public-sector | `govuk-frontend` | Legally / regulatorily expected |
| US public-sector / trust-first | `uswds` | Same |
| Fast agency MVP | Bootstrap 5.3 | Boring, fast, works |
| React foundation with accessibility | `@radix-ui/themes` | Primitives + polished theme |
| Modern SaaS (owned components) | shadcn/ui | You own the code; never ship default state |
| Default modern SaaS / AI marketing | Tailwind v4 utilities + `dark:` variant | Default for indie / small-team builds |
...
When the brief is an aesthetic, not a system (glassmorphism, bento, brutalism, editorial, dark tech, aurora, kinetic typography, Apple Liquid Glass), build with native CSS + Tailwind + a maintained component library, and be honest in code comments about what is borrowed inspiration vs. official material.
...
- Global state ONLY for deep prop-drilling avoidance (Zustand, Jotai, React context).
- **NEVER use `useState` for continuous values driven by user input** (mouse position, scroll progress, pointer physics, magnetic hover). Use Motion's `useMotionValue` / `useTransform` / `useScroll`. `useState` re-renders the React tree on every change and collapses on mobile.

### Icons
**Allowed libraries (priority order):** `@phosphor-icons/react`, `hugeicons-react`, `@radix-ui/react-icons`, `@tabler/icons-react`.
**Discouraged:** `lucide-react` (acceptable only on explicit request or if already a project dependency).
**NEVER hand-roll SVG icons.** If a glyph is missing, install a second library or compose from primitives.
**One family per project.** Do not mix Phosphor with Lucide in the same tree.
...
**Display / Headlines:**
- Default `text-4xl md:text-5xl lg:text-6xl tracking-tighter leading-none`
- Avoid oversized H1s. Control hierarchy with weight + color, not raw scale alone.
...
For premium-consumer briefs (cookware, wellness, artisan, luxury, heritage craft, DTC home goods, etc.), the LLM default is **warm beige/cream + brass/clay/oxblood/ochre + espresso/ink dark text**. This palette is **BANNED as the default reach**. Every premium-consumer site uses this exact palette. The brand becomes invisible.

**Banned hex families as default backgrounds and accents:**
...
**Palette-rotation rule:** if the previous premium-consumer project used the beige+brass family, this one MUST use a different family. Do not ship the same warm-craft palette twice in a row.
...
- **NO DUPLICATE CTA INTENT (MANDATORY):** Two CTAs with the same intent on one page is a Pre-Flight Fail. Examples: "Get in touch" + "Contact us" + "Let's talk" + "Start a project" + "Start something" + "Reach out" = all "contact" intent → pick ONE label and use it everywhere on the page. Same for "Try free" + "Get started" + "Sign up free" (all "signup" intent) and "View work" + "See selected work" + "Browse projects" (all "portfolio" intent). One label per intent.
...
- **GSAP Sticky-Stack Pattern (when scroll-stack is used).** A "card stack on scroll" must be a REAL sticky-stack, not a sequential reveal list. Critical points: `start: "top top"`, `pin: true`, every card except the last is pinned; the scale/opacity transform is driven by the NEXT card's scroll trigger (so previous card shrinks as next one arrives).
- **GSAP Horizontal-Pan Pattern (when horizontal scroll-hijack is used).** Critical points: `start: "top top"`, `pin: true`, `end: "+=${distance}"` (scroll length = horizontal travel needed), `scrub: 1`. The wrapper is pinned; the inner track slides horizontally as the user scrolls vertically.
- **Scroll-Reveal Stagger (lighter alternative).** For simple "items appear as they enter viewport" (no pinning), prefer Motion's `whileInView` over GSAP—lighter, no ScrollTrigger needed.
...
- Maintain visual hierarchy, brand identity, and WCAG AA contrast (AAA for body) across both modes.
...
- **8–10 (Advanced Choreography):** Complex scroll-triggered reveals, parallax, scroll-driven animation (CSS `animation-timeline` or GSAP ScrollTrigger). Use Motion hooks. **NEVER use `window.addEventListener('scroll')`**.
...
- [ ] **GSAP sticky-stack / horizontal-pan** implemented per Section 5 canonical skeleton (`start: "top top"`, `pin: true`, correct scrub)?
...
- W3C: https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/
...
Apple documents Liquid Glass inside Apple's Human Interface Guidelines and Developer Documentation for **Apple platforms only**. It is a dynamic material used across Apple platform UI. Apple's native implementation belongs to Apple platform APIs and system components, **not a public web CSS package**.

There is no `liquid-glass.css` from Apple for normal websites.

A web approximation can use `backdrop-filter`, transparent backgrounds, layered borders, highlight overlays, gradients, motion, and strong contrast fallbacks. But that is **web glassmorphism / frosted-glass approximation**, not official Apple Liquid Glass. Label it as such in comments and provide a solid-fill fallback under `prefers-reduced-transparency`.